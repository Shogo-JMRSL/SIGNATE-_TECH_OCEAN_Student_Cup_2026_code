...
主解析のコードは [scripts/run_segment_feature_experiments.py](scripts/run_segment_feature_experiments.py) です。比較の結果、基本セグメントに条件の重なりを加えたv2を採用しました。提出予測の保存先は `submissions/segment_v2/submission_segment_v2.csv` です。

このリポジトリには分析コードと実行手順をまとめています。データの再配布を避けるため、提出CSV、元データ、課題PDF、ノートブック、学習済みモデル、集計結果は含めていません。
